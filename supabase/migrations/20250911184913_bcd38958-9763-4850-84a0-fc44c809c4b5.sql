...
CREATE TYPE public.user_role AS ENUM ('admin', 'accountant');
...
CREATE TABLE public.merchants (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  merchant_type TEXT NOT NULL CHECK (merchant_type IN ('paytm', 'phonepe', 'gpay', 'bharatpe', 'pinelab')),
  qr_code_url TEXT,
  mobile_number TEXT,
  email_id TEXT,
  password TEXT,
...
CREATE POLICY "Users can view debit cards"
ON public.debit_cards FOR SELECT
USING (
  EXISTS (
    SELECT 1 FROM public.accounts a 
    WHERE a.id = debit_cards.account_id 
    AND auth.uid() IS NOT NULL
  )
);

CREATE POLICY "Users can manage debit cards"
ON public.debit_cards FOR ALL
USING (
  EXISTS (
    SELECT 1 FROM public.accounts a 
    WHERE a.id = debit_cards.account_id 
...
CREATE POLICY "Users can manage merchants"
...
CREATE TRIGGER update_debit_cards_updated_at
